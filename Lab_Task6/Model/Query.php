...
{
    $conn = Connection_db();
    $query = 'INSERT INTO user_info (Full_Name, Username, Email, Password, Gender, Date_of_birth)
...
{
    $conn = Connection_db();
    $selectQuery = "UPDATE user_info SET Full_Name = ?, Username = ?, Email = ?,Date_of_Birth = ? where ID = ?";
    try {
        $stmt = $conn->prepare($selectQuery);
        $stmt->execute([
